package platform

import "github.com/veraison/psatoken"

// TBBRoTPKey represents a single item in the CCA platform TBB ROTPK claim.
type TBBRoTPKey struct {
	Name        *string `cbor:"1,keyasint" json:"description"`  // e.g. "CM" or "DM"
	ActiveArray *int32  `cbor:"2,keyasint" json:"active-array"` // active ROTPK array
	Index       *int32  `cbor:"3,keyasint" json:"index"`        // index in the active array
	Hash        *[]byte `cbor:"4,keyasint" json:"hash"`         // hash object
}

func (i TBBRoTPKey) Validate() error {
	return ValidateTBBRoTPKey(&i)
}

func (i TBBRoTPKey) GetName() (string, error) {
	if i.Name == nil {
		return "", psatoken.ErrMandatoryFieldMissing
	}

	return *i.Name, nil
}

func (i TBBRoTPKey) GetActiveRoTPKArray() (int32, error) {
	if i.ActiveArray == nil {
		return 0, psatoken.ErrMandatoryFieldMissing
	}

	return *i.ActiveArray, nil
}

func (i TBBRoTPKey) GetIndex() (int32, error) {
	if i.Index == nil {
		return 0, psatoken.ErrMandatoryFieldMissing
	}

	return *i.Index, nil
}

func (i TBBRoTPKey) GetHash() ([]byte, error) {
	if i.Hash == nil {
		return nil, psatoken.ErrMandatoryFieldMissing
	}

	if err := psatoken.ValidatePSAHashType(*i.Hash); err != nil {
		return nil, err
	}

	return *i.Hash, nil
}

func (i *TBBRoTPKey) SetName(v string) error {
	i.Name = &v
	return nil
}

func (i *TBBRoTPKey) SetActiveRoTPKArray(v int32) error {
	i.ActiveArray = &v
	return nil
}

func (i *TBBRoTPKey) SetIndex(v int32) error {
	i.Index = &v
	return nil
}

func (i *TBBRoTPKey) SetHash(v []byte) error {
	if err := psatoken.ValidatePSAHashType(v); err != nil {
		return err
	}

	i.Hash = &v

	return nil
}
